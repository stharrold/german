"""Tests for A1 exam exercise content validation."""
...
A1_DIR = Path(__file__).parent.parent / "resources" / "exams" / "a1"
EXERCISE_ID_PATTERN = re.compile(r"^a1-(hoeren|lesen|schreiben|sprechen)-(teil|aufgabe)-\d+-\d{3}$")


def test_a1_meta_json_exists():
    """Test that meta.json exists for A1 exam."""
    assert (A1_DIR / "meta.json").exists()


def test_a1_meta_json_valid():
    """Test that A1 meta.json validates against ExamMeta model."""
    meta = load_exam_meta("a1")
    assert isinstance(meta, ExamMeta)
    assert meta.level == "A1"
    assert meta.provider == "Goethe-Institut"


def test_a1_directory_structure():
    """Test that all expected A1 exam directories exist."""
    expected_dirs = [
        "hoeren/teil-1", "hoeren/teil-2", "hoeren/teil-3",
...
def test_a1_lesen_exercises_exist(teil):
    """Test that each Lesen teil has 5 exercises."""
    teil_dir = A1_DIR / "lesen" / f"teil-{teil}"
    exercises = sorted(teil_dir.glob("uebung-*.json"))
    assert len(exercises) == 5, f"Expected 5 exercises in teil-{teil}, found {len(exercises)}"


@pytest.mark.parametrize("teil", [1, 2, 3, 4])
def test_a1_lesen_exercises_valid(teil):
...
def test_a1_schreiben_exercises_exist(aufgabe):
...
@pytest.mark.parametrize("aufgabe", [1, 2])
def test_a1_schreiben_exercises_valid(aufgabe):
...
@pytest.mark.parametrize("teil", [1, 2, 3])
def test_a1_sprechen_exercises_exist(teil):
    """Test that each Sprechen teil has 5 exercises."""
    teil_dir = A1_DIR / "sprechen" / f"teil-{teil}"
    exercises = sorted(teil_dir.glob("uebung-*.json"))
    assert len(exercises) == 5, f"Expected 5 exercises in teil-{teil}, found {len(exercises)}"


@pytest.mark.parametrize("teil", [1, 2, 3])
def test_a1_sprechen_exercises_valid(teil):
...
def test_a1_total_exercise_count():
    """Test that exactly 60 A1 exercises exist."""
    count = len(list(A1_DIR.glob("**/uebung-*.json")))
    assert count == 60, f"Expected 60 exercises, found {count}"


def test_a1_no_unicode_escapes():
    """Test that JSON files use native UTF-8, not unicode escapes."""
    for f in A1_DIR.glob("**/uebung-*.json"):
        content = f.read_text(encoding="utf-8")
        assert "\\u00" not in content, f"Unicode escape found in {f}"


def test_a1_all_ids_unique():
    """Test that all exercise IDs are unique across A1."""
    ids = []
    for f in A1_DIR.glob("**/uebung-*.json"):
