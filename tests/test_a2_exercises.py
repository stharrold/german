"""Tests for A2 exam exercise content validation."""
...
A2_DIR = Path(__file__).parent.parent / "resources" / "exams" / "a2"
EXERCISE_ID_PATTERN = re.compile(r"^a2-(hoeren|lesen|schreiben|sprechen)-(teil|aufgabe)-\d+-\d{3}$")


def test_a2_meta_json_exists():
    """Test that meta.json exists for A2 exam."""
    assert (A2_DIR / "meta.json").exists()


def test_a2_meta_json_valid():
    """Test that A2 meta.json validates against ExamMeta model."""
    meta = load_exam_meta("a2")
    assert isinstance(meta, ExamMeta)
    assert meta.level == "A2"
    assert meta.provider == "Goethe-Institut"


def test_a2_directory_structure():
    """Test that all expected A2 exam directories exist."""
    expected_dirs = [
    ...
    for subdir in expected_dirs:
        assert (A2_DIR / subdir).is_dir(), f"Missing directory: {A2_DIR / subdir}"
...
def test_a2_hoeren_exercises_exist(teil):
    """Test that each Hören teil has 7 exercises."""
...
@pytest.mark.parametrize("teil", [1, 2, 3, 4])
def test_a2_hoeren_exercises_valid(teil):
...
def test_a2_lesen_exercises_exist(teil):
    """Test that each Lesen teil has 7 exercises."""
...
@pytest.mark.parametrize("teil", [1, 2, 3, 4])
def test_a2_lesen_exercises_valid(teil):
...
def test_a2_schreiben_exercises_exist(aufgabe):
    """Test that each Schreiben aufgabe has 7 exercises."""
...
@pytest.mark.parametrize("aufgabe", [1, 2])
def test_a2_schreiben_exercises_valid(aufgabe):
...
def test_a2_sprechen_exercises_exist(teil):
    """Test that each Sprechen teil has 7 exercises."""
...
@pytest.mark.parametrize("teil", [1, 2, 3])
def test_a2_sprechen_exercises_valid(teil):
...
def test_a2_total_exercise_count():
    """Test that exactly 91 A2 exercises exist."""
...
def test_a2_all_ids_unique():
    """Test that all exercise IDs are unique across A2."""
    ids = []
    for f in A2_DIR.glob("**/uebung-*.json"):
