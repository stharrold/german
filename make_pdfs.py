...
Usage:
    uv run --extra pdf python make_pdfs.py
    uv run --extra pdf python make_pdfs.py --output-dir outputs/pdfs
    uv run --extra pdf python make_pdfs.py --skill hoeren
...
RESOURCES_DIR = Path(__file__).parent / "resources" / "exams" / "b1"
        ...
        for q in questions:
            ...
            if q_type == "true_false":
                ...
                if show_answers:
                    ...
                    self.cell(8, 6, "")
                    self.multi_cell(0, 6, f"{answer} -{explanation}")
                    self.set_text_color(0, 0, 0)
                else:
